...
double f(int x, int y, int z, double *phi1, double *phi2, double t){
	...
	return (D*laplaciano1-((phi1[Ny*(Nx*z+x)+y]*2.0*V0/(pow(phi0,2)))*(((pow(phi1[Ny*(Nx*z+x)+y],2)+pow(phi2[Ny*(Nx*z+x)+y],2))/(pow(phi0,2)))-1.0)));
}

double g(int x, int y, int z, double *phi1, double *phi2, double t){
	...
	return (D*laplaciano2-((phi2[Ny*(Nx*z+x)+y]*2.0*V0/(pow(phi0,2)))*(((pow(phi1[Ny*(Nx*z+x)+y],2)+pow(phi2[Ny*(Nx*z+x)+y],2))/(pow(phi0,2)))-1.0)));
}
...
int main(int argc, char **argv){
	...
	double *phi, *phi1_final, *phi2_final, *grad1x, *grad2x, *grad1y, *grad2y, *grad1z, *grad2z, *phi1, *phi2, *phi1_temp, *phi2_temp, *V;
...
//solução - começo
	...
	for (t = 1.0; t < tf; t += 0.05){
		for (z = 0; z < Nz; z += 1){
			for(x= 0; x< Nx; x++){
				for(y= 0; y< Ny; y++){
					phi1_temp[Ny*(Nx*z+x)+y]= phi1[Ny*(Nx*z+x)+y]+0.5*dt*f(x, y, z, phi1, phi2, t);
					phi2_temp[Ny*(Nx*z+x)+y]= phi2[Ny*(Nx*z+x)+y]+0.5*dt*g(x, y, z, phi1, phi2, t);
				}
			}
		}
		for (z = 0; z < Nz; z += 1){
			for(x= 0; x< Nx; x++){
				for(y= 0; y< Ny; y++){
					phi1[Ny*(Nx*z+x)+y]+= dt*f(x, y, z, phi1_temp, phi2_temp, t+0.5*dt);
					phi2[Ny*(Nx*z+x)+y]+= dt*g(x, y, z, phi1_temp, phi2_temp, t+0.5*dt);
				}
			}
		}
//normalização-começo
